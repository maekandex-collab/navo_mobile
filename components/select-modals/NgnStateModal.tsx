...
const NgnStateModal = ({placeholder, header, showModal, close, selectedValue, title, handlePress, handleShowModal}: {placeholder: string; header: string; showModal: boolean; close: () => void; selectedValue: string; title: string; handlePress: (value: string) => void, handleShowModal: () => void}) => {
                        ...
                        loading ? (
                            ...
                            <FlatList
                                ...
                                renderItem={({item, index}: {item: string; index: number}) => (
                                    <TouchableOpacity key={index} onPress={() => handlePress(item)} className='my-3 w-full'>
                                        <Text className='text-lg text-blue font-amedium'>{item}</Text>
                                    </TouchableOpacity>
                                )}
