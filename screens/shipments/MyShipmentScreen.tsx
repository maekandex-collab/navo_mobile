...
const MyShipmentScreen = () => {
  ...
  const getShipments = async () => {
      ...
      try {
        ...
        const result = await axiosClient.get("/shipments")

        console.log("ship=",result.data?.data[0]?.transaction)
  ...
  const performSearch = async (searchTerm: string) => {
    ...
    setLoading(true)
      ...
      try {
        ...
        const result = await axiosClient.get(`/shipments/search?q=${searchTerm}`)
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get(`/shipments?page=${nextPage}&limit=20`);
  ...
  const renderShipment = ({item, index}: {item: any, index: number}) => (
    <ShipmentCard item={item} index={index} handlePress={() => router.push({
  ...
  return (
    <SafeAreaView className='h-full bg-white px-4'>
      <Header title="My Shipments" showGoBack={true} onpress={() => router.back()}/>
      ...
      {shipment?.length === 0 && !loading ?
        <View className='flex-1'>
          ...
          <View className='w-full flex-1 items-center justify-center'>
            ...
            {query ? (
              ...
              <>
                <Text className="text-2xl text-center text-blue mt-4 font-abold">You have no</Text>
        ...
        <View className={`pt-4 ${Platform.OS == "ios" ? "pb-[125px]" : "pb-[145px]"}`}>
          ...
          <View className='flex flex-row w-full'>
            <SearchInput placeholder="Search Shipments..."  value={query} handleChangeText={(text) => setQuery(text)} disabled={shipment.length !== 0} otherStyles='w-4/5'/>
...
export default MyShipmentScreen