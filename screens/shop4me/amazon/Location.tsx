...
export default function Location() {
  ...
  const { cartTotal, productDetails } = useLocalSearchParams() as any;
  ...
  const submit = async () => {
      ...
      try {
        ...
        const result = await axiosClient.post("/amazonshop/add-to-cart", data)

        console.log("add amazon=",result.data)
        const cartId =  result.data.cartId;
        const response = await axiosClient.get(`/amazonshop/user-cart/${cartId}`)
        console.log("res=",response.data.items)

        setCost(response.data || null)
  ...
  const pay = async (payType: string) => {
    ...
    if(payType === 'NGN-WALLET'){
      ...
      try {
        ...
        const result = await axiosClient.post(`/amazonshop/checkout/${cost?.id}`, {
          paymentMethod: "nomba"
        })
        ...
        <BottomSheetFlatList
          ListHeaderComponent={() => (
            <View className='flex-col w-full items-center justify-between my-3'>
              ...
              <Text className="text-2xl text-center text-blue font-abold">{displayCurrency(Number(cost?.summary?.totalAmountNGN), "NGN")}</Text>
              <Text className="text-sm text-center text-blue font-amedium">1 GBP = {cost?.summary?.exchangeRate} NGN</Text>
            </View>
          )}
          data={cost?.items || []}
          keyExtractor={(item, index) => index.toString()}
          renderItem={({ item }) => (
            <ShopDetails itemName={item?.title} priceGBP={displayCurrency(Number(item?.conversion?.originalPriceGBP), "GBP")} priceNGN={displayCurrency(Number(item?.conversion?.originalPriceNGN), "NGN")} serviceFee={displayCurrency(Number(item?.conversion?.serviceFee), "NGN")} subTotal={displayCurrency(Number(item?.conversion?.totalAmountNGN), "NGN")} quantity={item?.quantity}/>
          )}
          showsVerticalScrollIndicator={false}
          ListFooterComponent={() => (
            <View>
              <Details title='Total price(GBP)' value={displayCurrency(Number(cost?.summary?.totalPriceGBP), "GBP")}/>
              <Details title='Total service charge' value={displayCurrency(Number(cost?.summary?.totalServiceFeeNGN), "NGN")}/>
              <Details title='Total price(NGN)' value={displayCurrency(Number(cost?.summary?.totalPriceNGN), "NGN")}/>
              <Details title='Total amount(NGN)' value={displayCurrency(Number(cost?.summary?.totalAmountNGN), "NGN")}/>
            </View>
          )}
        />
