...
import FoodingCard from '@/components/orders-card/FoodingCard'

const FoodingOrdersScreen = () => {
  ...
  const getOrders = async () => {
      ...
      try {
        ...
        const result = await axiosClient.get("/orders?category=ORDER")
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get(`/orders?category=ORDER&page=${nextPage}&limit=20`);
  ...
  const renderorders = ({item, index}: {item: any, index: number}) => (
    <FoodingCard item={item}/>
  )

  return (
    <SafeAreaView className='h-full bg-white px-4'>
      <Header title="Fooding" showGoBack={true} onpress={() => router.back()}/>
      ...
      {orders?.length === 0 && !loading ?
        <View className='flex-1'>
          <View className='w-full flex-1 items-center justify-center'>
            ...
            <Text className="text-2xl text-center text-blue font-abold">fooding orders yet!</Text>
         </View>

          <CustomButton title='Go to Fooding' containerStyles='mb-4' textStyles='text-white' handlePress={() => router.push("/(protected)/(routes)/Store")}/>
        </View> :
        <View>   
          <View>
            {
...
export default FoodingOrdersScreen