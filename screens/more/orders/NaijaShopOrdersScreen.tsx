...
import NaijaShopCard from '@/components/orders-card/NaijaShopCard'

const NaijaShopOrdersScreen = () => {
  ...
  const getOrders = async () => {
      ...
      try {
        ...
        const result = await axiosClient.get("/orders?category=VENDORS")
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get(`/orders?category=VENDORS&page=${nextPage}&limit=20`);
  ...
  const renderorders = ({item, index}: {item: any, index: number}) => (
    <NaijaShopCard item={item}/>
  )

  return (
    <SafeAreaView className='h-full bg-white px-4'>
      <Header title="Naija Shop" showGoBack={true} onpress={() => router.back()}/>
      ...
      {orders?.length === 0 && !loading ?
        <View className='flex-1'>
          <View className='w-full flex-1 items-center justify-center'>
            ...
            <Text className="text-2xl text-center text-blue font-abold">Naija shop orders yet!</Text>
         </View>

          <CustomButton title='Go to Naija Shop' containerStyles='mb-4' textStyles='text-white' handlePress={() => router.push("/(protected)/(routes)/NaijaShop")}/>
        </View> :
        <View>   
          <View>
            {
...
export default NaijaShopOrdersScreen