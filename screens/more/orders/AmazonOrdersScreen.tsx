...
import AmazonCard from '@/components/orders-card/AmazonCard'
...
const AmazonOrdersScreen = () => {
  ...
  const getOrders = async () => {
      ...
      try {
        ...
        const result = await axiosClient.get("/orders?category=CARTS")
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get(`/orders?category=CARTS&page=${nextPage}&limit=20`);
  ...
  const renderorders = ({item, index}: {item: any, index: number}) => (
    <AmazonCard item={item}/>
  )

  return (
    <SafeAreaView className='h-full bg-white px-4'>
      <Header title="Amazon" showGoBack={true} onpress={() => router.back()}/>
      ...
      {orders?.length === 0 && !loading ?
        <View className='flex-1'>
          <View className='w-full flex-1 items-center justify-center'>
            ...
            <Text className="text-2xl text-center text-blue font-abold">amazon orders yet!</Text>
         </View>

          <CustomButton title='Go to Amazon' containerStyles='mb-4' textStyles='text-white' handlePress={() => router.push("/(protected)/(routes)/Amazon")}/>
        </View> :
        <View>   
          <View>
            {
