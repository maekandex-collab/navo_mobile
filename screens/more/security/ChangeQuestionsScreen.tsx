...
const ChangeQuestionsScreen = () => {
  ...
  const submit = async () => {
    ...
    const data = {
     ...
     try {
          ...
          const result = await axiosClient.put("/secure/change-security-questions", data)
  ...
  return (
    <SafeAreaView className="bg-white h-full px-4">
      <Header title="Change Questions" showGoBack={true} onpress={() => router.back()}/>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {!loading && questions?.length !== 0 &&
            <View className="w-full justify-center my-4">
              <Text className="text-lg text-orange font-amedium text-center mb-2">Only two questions can be rechange!</Text>
              <Accordion
                sections={questions}
...
export default ChangeQuestionsScreen