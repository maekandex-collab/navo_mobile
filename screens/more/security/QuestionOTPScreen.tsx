...
const QuestionOTPScreen = () => {
    ...
    if(!userProfile?.isQuestionSet){
      ...
      try {
        ...
        const data = {
          ...
          resetQuestionOtp: otp
        }
        
        const result = await axiosClient.post("/secure/verify-secure-question-reset", data)
        ...
        router.replace("/(protected)/(routes)/SetQuestions")
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      const result = await axiosClient.post("/secure/resend-security-question-otp", data)
  ...
  return (
    <SafeAreaView className="bg-white h-full px-4">
      <Header title="Question OTP" showGoBack={true} onpress={() => router.back()}/>
...
export default QuestionOTPScreen
